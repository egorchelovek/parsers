from django.conf import settings
from app.parsers.utils import *
# import settings
# from utils import *
from selenium import webdriver
from selenium.common.exceptions import StaleElementReferenceException, NoSuchElementException
from time import sleep
import xlsxwriter
import re
from selenium.webdriver.chrome.options import Options
from os import path

def parse_youla(params):
    (TYPE, NUMBER_OF_ELEMENTS, CITY, ROOM_AREA_MIN, ROOM_AREA_MAX, MIN_PRICE, MAX_PRICE) = params
    FILENAME = 'youla.xlsx'

    # setup browser
    driver = webdriver.Chrome(chrome_options=get_options())
    driver = facking_location(driver, CITY)

    # authorize
    driver.get('https://vk.com/')
    driver.find_element_by_id('index_email').send_keys(settings.VK_CREDENTIALS['email'])
    driver.find_element_by_id('index_pass').send_keys(settings.VK_CREDENTIALS['pass'])
    driver.find_element_by_id('index_login_button').click()
    sleep(1)
    driver.get('https://youla.ru/login')
    sleep(1)
    driver.get('https://oauth.vk.com/authorize?response_type=code&client_id=5028682&scope=offline&redirect_uri=https%3A%2F%2Fyoula.ru%2Flogin%2Fvk')
    sleep(1)

    # get page
    SITE = key_by_value(settings.SOURCE_SITES, 'Youla')
    PAGE = settings.SOURCE_PAGES_RENT['Youla'] if TYPE == 0 else settings.SOURCE_PAGES_SELL['Youla']
    driver.get(SITE+PAGE)
    print(SITE+PAGE)

    # get first elements
    els = driver.find_elements_by_class_name('product_item')

    # get additional elements by scrolling page (works slowly)
    # if len(els) < NUMBER_OF_ELEMENTS:
    #     itr = 0
    #     while(len(els) >= NUMBER_OF_ELEMENTS or itr >= settings.MAX_SCROLLS):
    #         # scrolling go down to the page
    #         driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
    #         # get elements
    #         els = driver.find_elements_by_class_name('item__line')
    #         # increase counter
    #         itr += 1

    # get info elements
    itr = 0
    hlinks = []
    for el in els:

        # get hyperlink
        try:
            hlink = el.find_element_by_tag_name('a')
            print(hlink.get_attribute('href'))
            hlinks.append(hlink.get_attribute('href'))
        except StaleElementReferenceException:
            pass

        break

        # control number of objects to load
        itr += 1
        if itr >= NUMBER_OF_ELEMENTS:
            break

    # prepare document
    workbook, worksheet = prepare_workbook(FILENAME)

    row = 1
    for hlink in hlinks:
        # go to page
        driver.get(hlink)
        # get phone (so much tricky!)
        driver.find_element_by_xpath('//button[@data-test-action="PhoneNumberClick"]').click()
        sleep(1)

        contact = re.compile('UserName[^Click].*>(.*?)</p><div').search(driver.page_source).group(0)
        contact = re.compile('>(.*?)<').search(contact).group(0).split(' ')[0][1:]
        phone = re.compile('\+[0-9]\s\([0-9]{3}\)\s[0-9]{3}-[0-9]{2}-[0-9]{2}').search(driver.page_source).group(0)

        # get summary
        adress = re.search(re.compile('ProductMap(.*?)Description'), driver.page_source).group(0)
        adress = re.search(re.compile('span(.*?)</span'), adress).group(0)
        adress = re.search(re.compile('>(.*?)<'), adress).group(0).replace('>','').replace('<','')
        square = re.search(re.compile(',\s(.*?)\sм²'), driver.page_source).group(0)[-6:].strip()
        price = driver.find_element_by_xpath('//span[@data-test-component="Price"]').text
        floor = '-' # TODO

        # write info into the table
        worksheet.write_string(row,0,adress)
        worksheet.write_string(row,1,square)
        worksheet.write_string(row,2,price)
        worksheet.write_string(row,3,floor)
        worksheet.write_string(row,4,phone)
        worksheet.write_string(row,5,contact)
        worksheet.write_url(row,6,hlink)
        row += 1

        print(adress)
        print(square)
        print(price)
        print(contact)
        print(phone)
        print('***')

    # autofit page
    for i in range(0,7):
        set_column_autowidth(worksheet,i)

    # close session
    workbook.close()
    driver.close()
    driver.quit()

    return path.realpath(FILENAME)

# test
if __name__ == "__main__":
    TYPE = 0
    NUMBER_OF_ELEMENTS = 10
    CITY = 'Moscow'
    ROOM_AREA_MIN = 10
    ROOM_AREA_MAX = 100
    MIN_PRICE = 1000
    MAX_PRICE = 10000
    params = (TYPE, NUMBER_OF_ELEMENTS, CITY, ROOM_AREA_MIN, ROOM_AREA_MAX, MIN_PRICE, MAX_PRICE)
    parse_youla(params)
